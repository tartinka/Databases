...
CREATE DATABASE flights_schedule;
use flights_schedule;
DROP TABLE flights;
...
SELECT 
    id, flights_new.from, cities.name
FROM
    (SELECT 
        id, `name` AS `from`, `to`
    FROM
    ...
    JOIN cities
    WHERE
        flights.from = cities.label) AS flights_new
        JOIN
    cities
WHERE
    flights_new.to = cities.label;
